...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONUP:
    {
        if (g_flag == 4)
        {
            // 자유선 그리기가 끝나면 좌표를 무효화하여 다음 그리기에 영향을 주지 않도록 합니다.
            g_x = -1;
            g_y = -1;
            break;
        }
    ...
    case WM_MOUSEMOVE:
    {
        // 마우스가 클라이언트 영역 위에 있을 때 WM_MOUSELEAVE 메시지를 요청합니다.
        // 이렇게 하면 마우스가 창을 벗어날 때 한 번만 메시지를 받을 수 있습니다.
        TRACKMOUSEEVENT tme;
        ...
        if (g_flag == 4 && (wParam & MK_LBUTTON))
        {
            HDC hdc = GetDC(hWnd);
            int x = LOWORD(lParam);
            int y = HIWORD(lParam);

            // g_x, g_y가 유효한 좌표일 때만 선을 그립니다.
            // 마우스를 뗐거나 창을 벗어났다가 다시 들어오면 g_x는 -1이 됩니다.
            if (g_x != -1)
            {
                HPEN myPen, osPen;
    ...
    case WM_MOUSELEAVE:
    {
        // 마우스가 창을 벗어나면, 마지막 좌표를 무효화하여 선이 이어지지 않게 합니다.
        g_x = -1;
        g_y = -1;
    }
    break;
